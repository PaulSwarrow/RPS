...
namespace BitBenderGames
{
	...
	public class MobilePickingController : MonoBehaviour
	{
		...
		[SerializeField] [Tooltip("Size of the snap units when snapToGrid is enabled.")] private float snapUnitSize = 1;
		...
		public UnityEventWithPositionAndTransform OnPickableTransformMoveEnded;
		...
		[SerializeField]
		[Tooltip(
			"When setting this to false, the OnPickableTransformSelect event will only be sent once when clicking on the same pickable repeatedly.")]
		private bool repeatEventSelectedOnClick = true;

		[SerializeField]
		[Tooltip(
			"Previous versions of this asset may have fired the OnPickableTransformMoveStarted too early, when it hasn't actually been moved.")]
		...
		public void Awake()
		{
			...
			{
				Debug.LogError(
					"No TouchInputController found in scene. Make sure this component exists and is attached to the MobileTouchCamera gameObject.");
			}
		}
		...
		private void SelectColliderInternal(Component collider, bool isDoubleClick, bool isLongTap)
		{
			...
			{
				if (collider == null || MobileTouchPickable.Get(collider) == null)
				{
		...
		{
			if (!enabled) return;
			if (TouchWrapper.IsPointerOverGameObject()) return;
		...
		public Component GetClosestColliderAtScreenPoint(Vector3 screenPoint, out Vector3 intersectionPoint)
		{
			...
			if (Physics.Raycast(camRay, out hitInfo, float.PositiveInfinity, ~LayerMask.GetMask("UI")) == true)
			{
		...
		private void RequestDragPickable(Component colliderComponent, Vector2 fingerDownPos, Vector3? intersectionPoint)
		{
			...
			{
				MobileTouchPickable mobileTouchPickable = MobileTouchPickable.Get(SelectedCollider);
				if (mobileTouchPickable != null)
				{
		//			if (!EditHomeState.Editor && !(mobileTouchPickable.Owner is CharacterEntity)) return;
		...
		private void RequestDragPickable(Vector3 fingerDownPos)
		{
			...
			bool isDragStartedOnSelection = SelectedCollider != null &&
			                                GetClosestColliderAtScreenPoint(fingerDownPos, out intersectionPoint) ==
			                                SelectedCollider;
			if (isDragStartedOnSelection == true)
			{
		...
		private void InputControllerOnDragStart(Vector3 clickPosition, bool isLongTap)
		{
			...
			if (isLongTap == true && touchInputController.LongTapStartsDrag == true)
			{
				...
				if (newCollider != null)
				{
					MobileTouchPickable newPickable = MobileTouchPickable.Get(newCollider);
					if (newPickable != null)
					{
		...
		private void InputControllerOnDragUpdate(Vector3 dragPosStart, Vector3 dragPosCurrent, Vector3 correctionOffset)
		{
			...
			if (CurrentlyDraggedTransform != null)
			{
				...
				draggedItemCustomOffset +=
					CurrentlyDraggedTransform.position -
					currentlyDraggedTransformPosition; //Accomodate for custom movements by user code that happen while an item is being dragged. E.g. this allows users to lift items slightly during a drag.
				...
				if (mobileTouchCam.CameraAxes == CameraPlaneAxes.XY_2D_SIDESCROLL)
				{
					hasMoved = ComputeDistance2d(CurrentlyDraggedTransform.position.x, CurrentlyDraggedTransform.position.y,
						           currentlyDraggedTransformPosition.x, currentlyDraggedTransformPosition.y) >
					           transformMovedDistanceThreshold;
				}
				else
				{
					hasMoved = ComputeDistance2d(CurrentlyDraggedTransform.position.x, CurrentlyDraggedTransform.position.z,
						           currentlyDraggedTransformPosition.x, currentlyDraggedTransformPosition.z) >
					           transformMovedDistanceThreshold;
				}
				if (hasMoved == true)
				{
		...
		{
			Debug.Log("OnSelectedColliderChanged: " + selectionCollider.gameObject.name);
			var mobileTouchPickable = MobileTouchPickable.Get(selectionCollider);
			if (mobileTouchPickable != null)
			{
		...
		private void OnSelectedColliderChangedExtended(SelectionAction selectionAction, Component selectionCollider,
			bool isDoubleClick, bool isLongTap)
		{
